...
#include "platform.h"
#include "macro.h"
extern float xPos, yPos, lastX, lastY;
...
static LRESULT CALLBACK msg_callback(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_MOUSEMOVE:
		...
		SetCursorPos(WINDOW_WIDTH / 2 + windowRect.left, WINDOW_HEIGHT / 2 + windowRect.top);
		SetCursor(NULL);
...
int window_init(int width, int height, LPCWSTR title)
{
	...
	RECT windowRect;
	GetWindowRect(window->h_window, &windowRect);
	SetCursorPos(WINDOW_WIDTH / 2 + windowRect.left, WINDOW_HEIGHT / 2 + windowRect.top);
	lastX = WINDOW_WIDTH / 2 + windowRect.left;
	lastY = WINDOW_HEIGHT / 2 + windowRect.top;
	SetCursor(NULL);
